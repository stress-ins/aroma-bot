export function createBlendConstructorModule(deps) {
  ...
  async function openSavedBlendDetail(savedId) {
    ...
    try {
      ...
      elements.draftDetail.innerHTML = `<div class="detail-grid">${renderBackButton()}${renderPanelError("Смесь не найдена", "Возможно, она была удалена или недоступна.")}</div>`;
    }
  }
  ...
  async function openSharedBlend(savedId) {
    ...
    try {
      ...
      elements.draftDetail.innerHTML = `<div class="detail-grid">${renderBackButton()}${renderPanelError("Смесь не найдена или была удалена", "Вернитесь к списку сохранённых смесей.")}</div>`;
    }
  }
