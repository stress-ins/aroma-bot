export function createRecommendationsModule(deps) {
  ...
  function _renderMoodStep() { return '<div class="reco-chips">' + MOOD_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.mood === o.value ? " active" : "") + '" data-action="recoSelectMood" data-args=\'' + JSON.stringify([o.value]) + '\'><i data-lucide="' + o.icon + '" class="reco-chip-icon"></i> ' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
