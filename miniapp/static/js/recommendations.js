export function createRecommendationsModule(deps) {
  ...
  function _renderMoodStep() { return '<div class="reco-chips">' + MOOD_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.mood === o.value ? " active" : "") + '" data-action="recoSelectMood" data-args=\'["\' + o.value + \'"]\'><i data-lucide="' + o.icon + '" class="reco-chip-icon"></i> ' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
  function _renderGoalStep() { return '<div class="reco-chips">' + GOAL_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.goal === o.value ? " active" : "") + '" data-action="recoSelectGoal" data-args=\'["\' + o.value + \'"]\'>' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
  function _renderSymptomsStep() { return '<p class="reco-hint">Необязательно. Перечислите через запятую.</p><textarea class="reco-textarea" placeholder="Головная боль, бессонница..." data-on-input="recoUpdateSymptoms" data-args=\'[]\'>' + _wizardData.symptoms.join(", ") + '</textarea>'; }
  function _renderAromaPrefsStep() { return '<p class="reco-hint">Выберите любимые ароматы (необязательно).</p><div class="reco-chips">' + AROMA_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.aroma_preferences.includes(o.value) ? " active" : "") + '" data-action="recoToggleAroma" data-args=\'["\' + o.value + \'"]\'>' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
  function _renderContraindicationsStep() { return '<p class="reco-hint">Необязательно.</p><textarea class="reco-textarea" placeholder="Беременность, аллергии..." data-on-input="recoUpdateContra" data-args=\'[]\'>' + escapeHtml(_wizardData.contraindications) + '</textarea>'; }
  function _renderResults() {
    const recs = _wizardResults.recommendations || [], advice = _wizardResults.general_advice || "";
    const cards = recs.map(rec => { const card = rec.card, img = card && card.image_url ? '<img class="reco-card-image" src="' + escapeHtml(card.image_url) + '" alt="' + escapeHtml(rec.name_ru) + '" loading="lazy">' : ""; return '<article class="reco-card">' + img + '<div class="reco-card-body"><h3 class="reco-card-title">' + escapeHtml(rec.name_ru) + '</h3><p class="reco-card-reason">' + escapeHtml(rec.reason) + '</p><div class="reco-card-practice"><strong>Практика на день:</strong><p>' + escapeHtml(rec.daily_practice) + '</p></div><div class="reco-card-actions">' + (rec.slug ? '<button class="reco-card-btn reco-card-btn-detail" data-action="openReference" data-args=\'["\' + escapeHtml(rec.slug) + \'","aromas"]\'>Подробнее</button><button class="reco-card-btn reco-card-btn-blend" data-action="openBlendConstructor">Добавить в смесь</button>' : '') + '</div></div></article>'; }).join("");
