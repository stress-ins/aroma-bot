export function createRecommendationsModule(deps) {
  ...
  function openRecommendationsWizard() { _wizardOpen = true; _wizardStep = 0; _wizardData = _empty(); _wizardResults = null; _wizardLoading = false; renderWizard(); enterDetailView(); }
  function closeRecommendationsWizard() { _wizardOpen = false; _wizardStep = 0; _wizardResults = null; _wizardLoading = false; syncMobileNavigation(); }
  ...
  function renderWizard() {
    ...
    const navBtn = isLast ? '<button class="reco-submit-btn" onclick="submitRecommendations()"' + (!canProceed ? " disabled" : "") + '>Подобрать масла</button>' : '<button class="reco-next-btn" onclick="recoWizardNext()"' + (!canProceed ? " disabled" : "") + '>Далее</button>';
  ...
  function _renderMoodStep() { return '<div class="reco-chips">' + MOOD_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.mood === o.value ? " active" : "") + '" onclick="recoSelectMood(\'' + o.value + '\')"><i data-lucide="' + o.icon + '" class="reco-chip-icon"></i> ' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
  function _renderGoalStep() { return '<div class="reco-chips">' + GOAL_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.goal === o.value ? " active" : "") + '" onclick="recoSelectGoal(\'' + o.value + '\')">' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
  function _renderSymptomsStep() { return '<p class="reco-hint">Необязательно. Перечислите через запятую.</p><textarea class="reco-textarea" placeholder="Головная боль, бессонница..." oninput="recoUpdateSymptoms(this.value)">' + _wizardData.symptoms.join(", ") + '</textarea>'; }
  function _renderAromaPrefsStep() { return '<p class="reco-hint">Выберите любимые ароматы (необязательно).</p><div class="reco-chips">' + AROMA_OPTIONS.map(o => '<button class="reco-chip' + (_wizardData.aroma_preferences.includes(o.value) ? " active" : "") + '" onclick="recoToggleAroma(\'' + o.value + '\')">' + escapeHtml(o.label) + '</button>').join("") + '</div>'; }
  function _renderContraindicationsStep() { return '<p class="reco-hint">Необязательно.</p><textarea class="reco-textarea" placeholder="Беременность, аллергии..." oninput="recoUpdateContra(this.value)">' + escapeHtml(_wizardData.contraindications) + '</textarea>'; }
  function _renderResults() {
    const recs = _wizardResults.recommendations || [], advice = _wizardResults.general_advice || "";
    const cards = recs.map(rec => { const card = rec.card, img = card && card.image_url ? '<img class="reco-card-image" src="' + escapeHtml(card.image_url) + '" alt="' + escapeHtml(rec.name_ru) + '" loading="lazy">' : ""; return '<article class="reco-card">' + img + '<div class="reco-card-body"><h3 class="reco-card-title">' + escapeHtml(rec.name_ru) + '</h3><p class="reco-card-reason">' + escapeHtml(rec.reason) + '</p><div class="reco-card-practice"><strong>Практика на день:</strong><p>' + escapeHtml(rec.daily_practice) + '</p></div><div class="reco-card-actions">' + (rec.slug ? '<button class="reco-card-btn reco-card-btn-detail" onclick="openReference(\'' + escapeHtml(rec.slug) + '\', \'aromas\')">Подробнее</button><button class="reco-card-btn reco-card-btn-blend" onclick="openBlendConstructor()">Добавить в смесь</button>' : '') + '</div></div></article>'; }).join("");
    elements.draftDetail.innerHTML = renderBackButton("closeRecommendationsWizard") + '<div class="reco-wizard"><h2 class="reco-step-title">Ваши масла</h2><div class="reco-results">' + cards + (advice ? '<div class="reco-general-advice"><p>' + escapeHtml(advice) + '</p></div>' : '') + '<button class="reco-restart-btn" onclick="openRecommendationsWizard()">Подобрать ещё</button></div></div>';
  }
  function recoWizardNext() { if (_wizardStep < 4) { _wizardStep++; renderWizard(); } }
  function recoWizardBack() { if (_wizardStep > 0) { _wizardStep--; renderWizard(); } }
