export function createPlansModule(deps) {
  ...
  async function openPlanDetail(draftId, platform, slot) {
    ...
    try {
      ...
      elements.draftDetail.innerHTML = `${renderBackButton()}${deps.renderPanelError ? deps.renderPanelError("Не удалось загрузить", "Произошла ошибка при загрузке публикации") : ""}`;
    }
  }
