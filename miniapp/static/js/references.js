...
export function createReferencesModule(deps) {
  ...
  function blendLaunchContent(toolId) {
    ...
    const tryFill = () => {
      attempts++;
      for (const sel of ["[data-create-content] [name=topic]", "[data-create-threads-series] [name=topic]", "[data-create-carousel] [name=topic]", "#createForm textarea"]) {
  ...
  function blendRegenerate(btn) {
    ...
    fetchJson("/api/blend-constructor/construct", { method: "POST", body: JSON.stringify(req) })
      .then(result => renderBlendResult(result))
      .catch(() => {
