...
КРИТИЧЕСКИ ВАЖНАЯ СТРУКТУРА:
- Ровно 3 секции. Слова УТРО, ДЕНЬ, ВЕЧЕР — каждое на отдельной строке перед текстом поста
- Пропустить любую секцию = провал задания. ВСЕ ТРИ ОБЯЗАТЕЛЬНЫ

УТРО (Провокация):
- Цель: остановить скролл, забайтить на обсуждение
- Спорное мнение или неожиданный факт. Сразу к делу, без «доброе утро»
- [Хлёсткий тезис] + [Почему] + [Вопрос, на который хочется ответить]

ДЕНЬ (Мясо):
- Цель: сохранения и репосты
- Один конкретный совет, который можно применить сегодня
- Пиши как рассказ, не как нумерованный список. Каждый пункт понятен без контекста
- Плохо: «Запиши одно, что сработало в теле». Хорошо: «Вечером вспомни, от чего телу стало легче за день»

ВЕЧЕР (Личное):
- Цель: лояльность, «жиза» в комментариях
- Личная история, факап или наблюдение из практики
- «Самое странное в моей работе...» или «Заметила, что клиенты часто...»
