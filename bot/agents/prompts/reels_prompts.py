...
Верни ответ строго как JSON-массив из 4 кадров (без markdown-обёртки, без текста до/после):

[
  {{
    "timecode": "0-3 сек",
    "scene": "что именно в кадре — предмет, детали, атмосфера",
    "angle": "ракурс камеры, движение",
    "prompt": "English prompt for image generation, vertical portrait composition, brand aesthetic, NO faces, NO text, NO typography"
  }}
]
...
{{
  ...
  "scenario": "сценарий 4 хронометражных блока — 0-3, 3-10, 10-20, 20-30 сек; для каждого: видеоряд + текст на экране + голос",
...
Верни ответ строго как JSON-массив из {n_frames} кадров (без markdown-обёртки, без текста до/после):
