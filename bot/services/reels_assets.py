...
def regenerate_reels_frame_asset(draft_id: str, frame_index: int) -> dict[str, object] | None:
    ...
    for idx, item in enumerate(storyboard):
        ...
        frame = dict(item)
        if idx == frame_index:
            prompt = str(frame.get("gemini_prompt", "")).strip()
            if not prompt:
                return None
            image = generate_gemini_image_sync(prompt, log_context="MiniApp reels frame regenerate")
            if not image:
                return None
            asset = save_reels_frame_asset(draft_id, frame_index, image, prompt=prompt)
