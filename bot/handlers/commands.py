...
WELCOME_TEXT = (
    ...
    "*/trends* — аналитика трендов \\(RU \\+ EN\\)\n"
    "*/content* — создать контент\\: цель → формат → тема → материал\n"
    "*/threads* — пост для Threads \\+ картинка\n"
    "*/carousel* — карусель из 5 слайдов\n"
    "*/reels* — сценарий для Reels\n"
    "*/adapt* — адаптация поста под платформу\n"
    "*/plan* — контент\\-план на неделю\n"
    "*/drafts* — последние черновики\n"
    "*/keywords* — ключевые слова\n"
    "*/app* — открыть Mini App\n"
    "*/status* — активные источники данных\n"
    "*/help* — список команд"
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and context.args[0].startswith("invite_"):
        ...
        result = await accept_invite(invite_code, update.effective_user.id, username=update.effective_user.username or "", first_name=update.effective_user.first_name or "")
        if result is None:
