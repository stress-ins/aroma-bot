...
class TestGenerateSlideImagePrompts:
    ...
    def _make_router_passthrough(self):
        """Return a mock that passes through raw prompts with a prefix."""
        def _fake_optimize(raw_prompt, model, *, topic="", slide_number=0, total_slides=1, user_note="", blend_mood=None):
            return f"OPTIMIZED_{slide_number}: {raw_prompt}"
        return _fake_optimize

    def _setup_carousel_mocks(self, monkeypatch, response):
        ...
        monkeypatch.setattr("bot.services.carousel_assets._get_carousel_model", lambda: "gpt-image/1.5-text-to-image")
        return c
    ...
    def test_returns_same_count_as_slides(self, monkeypatch):
        ...
        c = self._setup_carousel_mocks(monkeypatch, response)
        monkeypatch.setattr(
            "bot.agents.image_prompt_router.optimize_image_prompt",
            self._make_router_passthrough(),
        )
    ...
    def test_fallback_on_empty_parse(self, monkeypatch):
        ...
        c = self._setup_carousel_mocks(monkeypatch, "ничего")
        monkeypatch.setattr(
            "bot.agents.image_prompt_router.optimize_image_prompt",
            self._make_router_passthrough(),
        )
    ...
    def test_image_router_called_for_each_slide(self, monkeypatch):
        """Every slide prompt passes through the image prompt router."""
        response = "\n".join(
        ...
        def _tracking_optimize(raw_prompt, model, *, topic="", slide_number=0, total_slides=1, user_note="", blend_mood=None):
    ...
    def test_prompt_includes_forbidden_visual_motifs(self, monkeypatch):
        ...
        def _noop_optimize(raw_prompt, model=None, **kw):
            return raw_prompt

        with _patch("bot.services.claude_client.call_claude", side_effect=_fake_call_claude), \
             _patch("bot.agents.image_prompt_router.optimize_image_prompt", side_effect=_noop_optimize), \
             _patch("bot.services.carousel_assets._get_carousel_model", return_value="gpt-image/1.5-text-to-image"):
