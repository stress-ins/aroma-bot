"""Tests: Claude client fallback to Kie.ai Gemini."""
...
def test_claude_fallback_to_kie_gemini():
    """When Claude retries exhausted and kie_ai_api_key set → fallback to Kie.ai Gemini."""
    with (
        ...
        patch(f"{_MODULE}._call_kie_gemini", return_value="gemini response") as mock_kie,
    ):
        ...
        # Should notify about fallback
        assert any("fallback" in str(c).lower() for c in mock_notify.call_args_list)


def test_claude_and_kie_both_fail():
    """When both Claude and Kie.ai fail → raise original Claude exception."""
    with (
        patch(f"{_MODULE}._get_client") as mock_client_fn,
        patch(f"{_MODULE}.settings") as mock_settings,
        patch("bot.handlers.monitor.notify_owner_throttled"),
        patch(f"{_MODULE}._call_kie_gemini", side_effect=ValueError("kie failed")),
    ):
        mock_settings.kie_ai_api_key = "test-key"
        mock_settings.anthropic_api_key = "test"
...
def test_no_fallback_without_kie_key():
    ...
    with (
        ...
        patch(f"{_MODULE}.settings") as mock_settings,
        patch("bot.handlers.monitor.notify_owner_throttled"),
        patch(f"{_MODULE}._call_kie_gemini") as mock_kie,
    ):
