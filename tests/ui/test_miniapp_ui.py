...
def test_each_tab_has_unique_content(page):
    ...
    assert page.get_by_text("Согласование", exact=True).is_visible()
    page.wait_for_timeout(300)
    ...
    page.wait_for_timeout(300)
    assert page.get_by_role("button", name="Справочник").is_visible()
    assert page.get_by_text("Доступ к справочнику ароматов ограничен.").count() >= 1
