...
Человек участвует ТОЛЬКО на UX-развилке: если есть выбор из 2+ принципиально разных дизайнов → один batched `AskUserQuestion` с превью. Критерии приёмки Claude формулирует сам по best-practice (фиксирует в PR description) — это больше не точка ожидания.

Единый набор стоп-точек — `~/.claude/CLAUDE.md` §«Стоп-точки (единый дефолт)»: необратимое уничтожение данных / секреты / billing / UX-развилка. Всё остальное автономно.
